...
// Ambil seluruh kategori dokumen
$categoryList = App\Models\PlanningDocumentCategory::where('opd_id', $opd?->id)->get();
...
<div class="w-full bg-slate-50/60 py-10 min-h-screen">
    <div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Filter Navigasi Tab Kategori -->
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
            <div class="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0">
                <a href="{{ url('/planning-dokumen') }}"
                    class="px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap bg-brand-500 text-white shadow-md shadow-brand-500/20">
                    Semua
                </a>
                @foreach($categoryList as $cat)
                <a href="{{ url('/planning-dokumen/kategori/' . $cat->slug) }}"
                    class="px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap bg-slate-100 text-slate-600 hover:bg-brand-500 hover:text-slate-100">
                    {{ $cat->title }}
                </a>
                @endforeach
            </div>
